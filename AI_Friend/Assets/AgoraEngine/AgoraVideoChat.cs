...
public class AgoraVideoChat : Photon.MonoBehaviour
{
    ...
    private void CreateUserVideoSurface(uint uid, bool isLocalUser)
    {
        ...
        content.sizeDelta = new Vector2(0, playerVideoList.Count * spaceBetweenUserVideos);
    ...
    private void RemoveUserVideoSurface(uint deletedUID)
    {
        ...
        //content.sizeDelta = oldContent + Vector2.right * spaceBetweenUserVideos; /* 비디오 스크롤방향 좌우일때 */ 
    ...
    private void UpdatePlayerVideoPostions()
    {
        for (int i = 0; i < playerVideoList.Count; i++)
        {
            ...
            //playerVideoList[i].GetComponent<RectTransform>().anchoredPosition = Vector2.right * spaceBetweenUserVideos * i; /* 비디오 스크롤방향 좌우일때 */ 
